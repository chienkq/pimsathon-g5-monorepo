...
/** Injected via `executeWorkflow(workflow, { services: { gitClient } })` — backend provides the real
 *  GitHub API implementation, using the `github` integration's Personal Access Token (see
 *  [[integrations_screen_w6]]). For a local-only checkout with no GitHub connection, use the
 *  separate Git node instead (`git.ts`, read-only, no credentials). */
...
export const githubNodeType: NodeTypeDefinition = {
  type: "github",
  displayName: "GitHub",
  description: "Reads or writes against a real GitHub repository — branches, commits, pull requests, issues.",
  group: "app",
  color: "#24292e",
  hasInput: true,
  outputs: ["main"],
  parameters: [
    ...
    { key: "owner", label: "Owner", type: "string", default: "", placeholder: "e.g. octocat" },
    { key: "repo", label: "Repository", type: "string", default: "", placeholder: "e.g. hello-world" },
    {
      key: "branch",
      label: "Branch",
      type: "string",
  ...
  async execute({ parameters, services }) {
    ...
    if (!gitClient) throw new Error("GitHub node requires a `gitClient` service (only available in backend).");

    const owner = String(parameters.owner ?? "");
    const repo = String(parameters.repo ?? "");
    if (!owner || !repo) throw new Error("GitHub node requires an Owner and a Repository.");
    ...
    switch (action) {
      ...
      case "Create Issue": {
        ...
        if (!title) throw new Error("GitHub Create Issue requires a Title.");
      ...
      case "Create Branch": {
        ...
        if (!name) throw new Error("GitHub Create Branch requires a New Branch Name.");
      ...
      case "Create Pull Request": {
        ...
        if (!title || !head) throw new Error("GitHub Create Pull Request requires a Title and a Head Branch.");
